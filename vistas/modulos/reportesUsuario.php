...
<div class="content-wrapper" style="min-height: 460.2px;">
    <section class="content-header">
      <h1>
        ...
        <small>Filtro: Rango Fechas | Gráfico por Usuario</small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="inicio"><i class="fa fa-dashboard"></i> Inicio</a></li>
        <li class="active">Tickets por Usuario</li>
      </ol>
    </section>
        ...
        <div class="col-xs-12">
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="box box-primary">
                <div class="box-header with-border">
                  <h3 class="box-title"><i class="fa fa-pie-chart"></i> Distribución por Usuario</h3>
                  <div class="box-tools pull-right">
                    <button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
                  </div>
                </div>
                <div class="box-body">
                  <canvas id="usuarioChart" width="800" height="400"></canvas>
          ...
          <div class="box box-solid box-primary">
            <div class="box-header with-border">
              <h3 class="box-title"><i class="fa fa-table"></i> Listado de tickets por usuario</h3>
              <div class="box-tools pull-right">
                <button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
              </div>
            </div>
              ...
              <table id="ticketsTable" class="table table-bordered table-striped table-hover dataTable dt-responsive tablas" role="grid">
                ...
                <tbody>
                  ...
                  foreach ($ticketusuario as $key => $value){
                    ...
                    $nombreUsuario = isset($value["id_usuario"]) ? $value["id_usuario"] : 'Sin asignar';
                  ?>
                  <tr>
                    <td><?php echo ($key+1); ?></td>
                    <td><?php echo $value["id_ticket"]; ?></td>
                    <td><?php echo obtenerDatosUsuario($value["id_usuario"])['nombre'] . ' ' . obtenerDatosUsuario($value["id_usuario"])['apellido']; ?></td>
  ...
  <script>
    $(document).ready(function() {
        ...
        function updateCounters() {
          ...
          // Calcular tickets únicos (sin duplicado) a partir de los datos visibles
          let uniqueTickets = new Set();
          // Recorremos solo las filas visibles (aplicando filtros actuales)
          table.rows({ search: 'applied' }).every(function(rowIdx, tableLoop, rowLoop) {
            let rowData = this.data();
            let ticketValue = rowData[1]; // La columna Ticket es la posición 1 (0-based)
            if (ticketValue && ticketValue.trim() !== '') {
              uniqueTickets.add(ticketValue);
            }
          });
          $('#uniqueTicketsCount').text(uniqueTickets.size);
        }
        ...
        function getVisibleData() {
          ...
          table.rows({ search: 'applied' }).every(function(rowIdx, tableLoop, rowLoop) {
            let rowData = this.data();
            let usuario = rowData[2];
            data.usuarios[usuario] = (data.usuarios[usuario] || 0) + 1;
          });
        ...
        function updateUsuarioChart() {
          let visibleData = getVisibleData();
          updateOrCreateChart(usuarioChart, 'usuarioChart', visibleData.usuarios, 'Distribución por Usuario');
        }
