<!-- MODAL EDITAR TICKET -->
<div class="modal fade" id="modalEditarTicket2" tabindex="-1" role="dialog" aria-labelledby="modalEditarTicketLabel">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <form role="form" method="post" enctype="multipart/form-data" id="formEditarTicket">
      
        <div class="modal-body">
          <div class="row">
            <div class="col-md-12">
              <!-- Select2 para ENTE - Agregado ID -->
              <div class="form-group col-md-6">
                <label for="editarEnte"><i class="fa fa-building"></i> Ente</label>
                <select class="form-control select2-ente-editar" name="editarEnte" style="width: 100%;" required>

                    <?php echo '<option value="'.$ticketInfo["id_ente"].'">'.obtenerNombreEnte($ticketInfo["id_ente"]).'</option>';?>
                
                    <?php
                    $entes = ControladorEntes::ctrMostrarEntes(null, null);
                    foreach ($entes as $key => $value){
                      if ($value["estado"] != "1") continue;
                      if ($value["id"] === $ticketInfo["id_ente"]) continue;
              ...
              <div class="form-group col-md-6">
                ...
                <input type="text" class="form-control" id="editarSolicitante" name="editarSolicitante" placeholder="Nombre del solicitante" value="<?php echo $ticketInfo["solicitante"];?>" required>
                <input type="hidden" class="form-control"  value="<?php echo $ticketInfo["ticket"];?>" name="editarTicket" required>
                <input type="hidden"  value="<?php echo $ticketInfo["id"];?>" name="idTicket">
              </div>
            </div>

            <div class="col-md-12">
              <!-- Select2 para MEDIO - Agregado ID -->
              <div class="form-group col-md-6">
                ...
                <select class="form-control select2-medio-editar" id="editarMedio" name="editarMedio" style="width: 100%;" required>
                    <?php echo '<option value="'.$ticketInfo["id_medio"].'">'.obtenerNombreMedio($ticketInfo["id_medio"]).'</option>';?>
                    <?php
                    $medio = ControladorMedio::ctrMostrarMedio(null, null);
                    foreach ($medio as $key => $valueMedio){
                      if ($valueMedio["estado"] != "1") continue;
                      if ($valueMedio["id"] === $ticketInfo["id_medio"]) continue;
              ...
              <!-- Select2 para PRIORIDAD - Agregado ID -->
              <div class="form-group col-md-6">
                ...
                <select class="form-control select2-prioridad-editar" id="editarPrioridad" name="editarPrioridad" style="width: 100%;" required>
                    ...
                    echo '<option value="'.$ticketInfo["id_prioridad"].'" data-color="'.$datosPrioridad["color"].'">' . $datosPrioridad["prioridad"] . '</option>';
                    ...
                    foreach ($prioridad as $key => $valuePrioridad){
                      ...
                      if ($valuePrioridad["id"] === $ticketInfo["id_prioridad"]) continue;
          ...
          <div class="row">
            <div class="col-md-12">     
              ...
              <div class="form-group col-md-12">
                ...
                <textarea class="form-control" id="editarDescripcion" name="editarDescripcion" rows="5" placeholder="Describa el problema o solicitud..." required><?php echo $ticketInfo["descripcion"]?></textarea>
              </div>
            </div>
          </div>
            ...
            <div class="col-md-12">
              ...
              <div class="form-group col-md-6">
                ...
                <select class="form-control select2-finalizado-editar" id="editarFinalizado" name="editarFinalizado" style="width: 100%;" required>
                  <option value="si" <?php echo (isset($ticketInfo["finalizado"]) && $ticketInfo["finalizado"] == "si") ? "selected" : ""; ?>>Sí</option>
                  <option value="no" <?php echo (isset($ticketInfo["finalizado"]) && $ticketInfo["finalizado"] == "no") ? "selected" : ""; ?>>No</option>
                </select>
              </div>
            </div>
          </div>
        ...
        <?php
        $editarTicket2 = new ControladorTicket();
        $editarTicket2 -> ctrEditarTicket2();
        ?>
...
<script>
  $(document).ready(function() {
    ...
    // Inicializar Select2 para el modal de EDICIÓN - CORREGIDO dropdownParent
    $('.select2-ente-editar').select2({
      theme: 'bootstrap',
      language: 'es',
      dropdownParent: $('#modalEditarTicket2')
    });
    ...
    // Inicializar Select2 para FINALIZADO
